...
def osc_pad(s: str | bytes) -> bytes:
    if isinstance(s, str):
        s = s.encode("utf-8")
    # OSC strings must be null-terminated and padded to a 4-byte boundary.
    pad = 4 - (len(s) % 4)
    return s + b"\x00" * pad
...
def list_midi_ports(binary: str = "amidi") -> list[str]:
    """List ALSA raw MIDI ports visible to amidi."""
    try:
        proc = subprocess.run([binary, "-l"], capture_output=True, text=True, check=True)
    except FileNotFoundError:
        raise RuntimeError("amidi is not installed") from None
    except subprocess.CalledProcessError as exc:
        stderr = exc.stderr.strip()
        raise RuntimeError(stderr or "amidi -l failed") from exc
...
class X32MidiSysexClient:
    """Write-only X32 transport over MIDI SysEx via amidi.

    The X32 protocol document describes OSC-over-MIDI SysEx messages for
    command transmission. Query/reply support is not documented the same way,
    so this transport intentionally limits itself to sending control commands.
    """

    def __init__(self, port: str, timeout: float = 1.5, binary: str = "amidi"):
        self.port = port
        self.timeout = timeout
        self.binary = binary
        self.supports_queries = False

    def _run_amidi(self, *args: str) -> None:
        try:
            subprocess.run(
                [self.binary, "-p", self.port, *args],
                capture_output=True,
                text=True,
                check=True,
            ...
            raise RuntimeError(f"timed out sending MIDI SysEx to {self.port}") from exc
        except subprocess.CalledProcessError as exc:
            stderr = exc.stderr.strip()
            raise RuntimeError(stderr or f"amidi send failed for {self.port}") from exc

    @staticmethod
    def _quote_string(val: str) -> str:
        if not val or any(ch.isspace() for ch in val) or '"' in val:
            escaped = val.replace("\\", "\\\\").replace('"', '\\"')
            return f'"{escaped}"'
        return val

    @classmethod
    def _text_value(cls, val: Any) -> str:
        if isinstance(val, bool):
            return "ON" if val else "OFF"
        if isinstance(val, str):
            return cls._quote_string(val)
        return str(val)

    @staticmethod
    def _matches_path(path: str, suffixes: tuple[str, ...]) -> bool:
        return any(path.endswith(suffix) for suffix in suffixes)

    @classmethod
    def _text_param(cls, path: str, val: Any) -> str:
        if isinstance(val, bool):
            return cls._text_value(val)

        # MIDI SysEx uses text values, so normalized API fields need to be
        # expanded to the mixer-facing units the X32 expects.
        if isinstance(val, int) and cls._matches_path(
            path,
            (
                "/on",
                "/st",
        ...
        if isinstance(val, (int, float)):
            if cls._matches_path(path, ("/mix/fader", "/mix/mlevel", "/fader", "/level")):
                return f"{_fader_to_db(float(val)):.1f}"
    ...
    @staticmethod
    def _sysex_hex(text_cmd: str) -> str:
        payload = text_cmd.encode("utf-8").hex().upper()
        return f"F000203232{payload}F7"

    def send_text(self, text_cmd: str) -> None:
        self._run_amidi("-S", self._sysex_hex(text_cmd))

    def _unsupported_query(self, path: str) -> None:
        raise NotImplementedError(
            f"MIDI SysEx transport is write-only here; query not supported for {path}"
        )

    def send(self, msg: bytes) -> None:
        addr, tt, vals = parse_osc(msg)
        types = tt.lstrip(",")
        if not types:
            self.send_text(addr)
            return
        if len(types) != len(vals):
            raise ValueError("cannot map OSC message to MIDI SysEx text command")
        text = " ".join([addr, *(self._text_param(addr, v) for v in vals)])
        self.send_text(text)

    def recv(self, timeout: Optional[float] = None) -> bytes:
        raise NotImplementedError("MIDI SysEx recv is not implemented")

    def get(self, path: str, retries: int = 3) -> Optional[list]:
        self._unsupported_query(path)

    def set(self, path: str, typetag: str, *values: Any) -> Optional[list]:
        text = " ".join([path, *(self._text_param(path, v) for v in values)])
        self.send_text(text)
        return None
...
class USB:
    ...
    def cd(self, path: str) -> None:
        """Navigate using a relative USB path like Dir/Subdir or .."""
    ...
    def upload(self, filename: str, scene_num: Optional[int] = None) -> None:
        """Upload/save a scene to USB.

        Sets the USB title to *filename* and triggers a save.
        If *scene_num* (0-99) is given, that scene is saved; otherwise
        the current scene is saved.

        Use usb.cd() first to navigate to the desired directory.
        """
        if scene_num is not None and (scene_num < 0 or scene_num > 99):
            raise ValueError("scene_num: 0-99")
        self.title = filename
        time.sleep(0.3)
        self._c.set_int("/-action/save", scene_num if scene_num is not None else 1)

    def upload_current(self, filename: str) -> None:
        """Upload/save the current scene to USB."""
    ...
    @property
    def tape_time(self) -> float:
        return self._c.get_float("/-stat/tape/time") or 0.0

    @property
    def tape_length(self) -> float:
        return self._c.get_float("/-stat/tape/len") or 0.0
...
class X32:
    ...
    def __init__(
        self,
        host: str = "192.168.0.64",
        port: int = 10023,
        timeout: float = 3.0,
        midi_port: Optional[str] = None,
    ):
        if midi_port is None and host.startswith("midi:"):
            midi_port = host.split(":", 1)[1]

        if midi_port:
            self._client = X32MidiSysexClient(midi_port, timeout)
        else:
...
def main():
    ...
    with X32(host) as x:
        print(f"X32 @ {host}")
        if getattr(x.client, "supports_queries", False):
            print(f"  Info: {x.info}")
            ...
            print("  MIDI SysEx mode: write-only control transport")
            print("  Example: x.ch(1).mute(), x.ch(1).fader = 0.75, x.go_scene(1)")
